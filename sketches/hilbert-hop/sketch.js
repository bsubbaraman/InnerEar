...
function fabDraw() {
  ...
  let offsetAmplitude = 1;
  let dataOffset;

  // try switching back and forth with a bool
  let dataFlag = false;

  for (let h = 0.2; h <= discHeight; h += layerHeight) {
    // rotate each layer by a random amount
    let theta = random(TWO_PI);
    // retract back to our start point
    let [xc, yc] = mapSquareToCircle(path[1].x, path[1].y);
    fab.moveRetract(xc, yc, h);
      ...
      switch (dir) {
        case 'r':
          for (let x = path[i-1].x; x <= path[i].x; x += dataSpacing) {
            ...
            if (dataOffset > 0) {
              dataOffset = offsetAmplitude;
            }

            let [xc, yc] = mapSquareToCircle(x, path[i].y);
            // try adding a 'somersault'
            if (dataOffset > 0) {
              fab.moveExtrude(xc, yc + dataOffset, h + dataOffset/4, s);
              fab.moveExtrude(xc + dataOffset, yc, h + dataOffset/4, s);
              fab.moveExtrude(xc, yc - dataOffset, h + dataOffset/4, s);
            }
            fab.moveExtrude(xc, yc, h + dataOffset/4, s);
        ...
        case 'l':
          for (let x = path[i-1].x; x >= path[i].x; x -= dataSpacing) {
            ...
            fab.moveExtrude(xc, yc, h + dataOffset, s);
        ...
        case 'u':
          for (let y = path[i-1].y; y <= path[i].y; y += dataSpacing) {
            ...
            let [xc, yc] = mapSquareToCircle(path[i].x, y);
            // try adding a 'somersault'
            if (dataOffset > 0) {
              fab.moveExtrude(xc + dataOffset, yc, h + dataOffset/4, s);
              fab.moveExtrude(xc, yc + dataOffset, h + dataOffset/4, s);
